import {evaluate} from "../_snowpack/pkg/mathjs.js";
const buttons = document.querySelectorAll("button");
const screen = document.getElementById("screen");
const lowScreen = document.getElementById("low-screen");
let ctrlHintEnable = true;
let timesPressedConsecutively;
const hintElement = document.querySelectorAll(".hint")[0];
...
function manageInput(key) {
  ...
  if (key === "*" || key === "/" || key === "-" || key === "+") {
    ...
    if (lastCharactor === "*" || lastCharactor === "/" || lastCharactor === "+" || lastTwoCharactors === "--") {
      lowScreen.innerText = lowScreen.innerText.slice(0, lowScreen.innerText.length - 1);
      lowScreen.textContent += key;
      return;
    }
